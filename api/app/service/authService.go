...
func NewAuthService(userRepo repository.UserRepositoryInterface, logger logger.Logger) AuthServiceInterface {
	return &AuthService{
		userRepo,
		logger,
	}
...
func (s *AuthService) Login(IDNumber string, password string) (refreshToken string, accessToken string, err error) {
	...
	matchPassword := helpers.CheckPasswordHash(password, user.Password)
